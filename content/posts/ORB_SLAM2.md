...
单目初始化分主要为4步：
1. 特征匹配 
2. 8点法多轮 ransac 求解，根据投影点到极线的距离选取最优结果
3. 利用求解的位姿结果进行地图点三角化。然后去除深度为负、无限远点(距离太远估计的深度值不准确)、视差太小、重投影误差的点 `Initializer::CheckRT`。
4. 根据计算重投影误差和视差选取最优的解。
...
## 2 回环检测
![12-ORB_SLAM.png](12-ORB_SLAM.png)
### 2.1 回环候选帧选取 
...
```。
1. ![11-ORB_SLAM.png](11-ORB_SLAM.png)
